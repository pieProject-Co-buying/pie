...
public class BusinessApplyController {
	...
	public String apply(HttpServletRequest request, Model model) {
		...
		dto.setBus_maxqnt(Integer.parseInt(request.getParameter("bus_maxqnt"))); //공구 수량
		dto.setBus_unitPrice(Integer.parseInt(request.getParameter("bus_unitPrice"))); //단가
		dto.setBus_chargePerson(request.getParameter("bus_chargePerson")); //담당자
		dto.setBus_phone(request.getParameter("bus_phone")); //전화번호
